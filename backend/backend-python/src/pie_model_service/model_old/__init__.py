...
def load_model_info(config: ServiceConfig) -> ModelInfo:
    ...
    model_path = Path(config.cache_dir) / "models"
    metadata_path = model_path / f"{config.model}.toml"

    if not metadata_path.exists():
        raise ValueError(
            f"Metadata file for model {config.model} not found at: {metadata_path}"
        )
...
def load_model(config: ServiceConfig, model_info: ModelInfo) -> torch.nn.Module:
    ...
    try:
        ...
        for param_file in tqdm(
            ...
            weights_path = model_path / config.model / param_file
